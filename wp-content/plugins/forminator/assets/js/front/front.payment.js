...
(function ($, window, document, undefined) {
	...
	$.extend(ForminatorFrontPayment.prototype, {
		...
		updateAmount: function(e, formData) {
			...
			updateFormData.set( 'mode', this.getStripeData('mode') );
			...
			$.ajax({
				...
				success: function (data) {
					if (data.success === true) {
						self.handleCardPayment(e, formData);
					} else {
						self.show_error(data.data);
					}
				},
				error: function (err) {
		...
		handleCardPayment: function (e, formData) {
			var self = this;
			var secret = this.getStripeData('secret');
